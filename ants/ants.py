...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place
        step = 0
        while step < self.min_range:
            place = place.entrance
            step += 1
        while step <= self.max_range and place is not hive:
            if len(place.bees):
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_queen:
            ...
            while p:
                current_ant = p.ant
                self.double_ant(current_ant)
                if current_ant and current_ant.is_container:
                    current_ant = current_ant.contained_ant
                    self.double_ant(current_ant)
                p = p.exit
        else:
            self.reduce_armor(1)
    ...
    def double_ant(self, ant):
        if ant and not ant in self.doubled_ant:
            ant.damage *= 2
            self.doubled_ant.append(ant)
...
def apply_effect(effect, bee, duration):
    ...
    def applying(colony):
        ...
        if duration <= 0:
            ...
            new_action(colony)
        duration -= 1
        if duration <= 0 and bee.action == applying:
