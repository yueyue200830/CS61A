...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    result = [[find_closest(restaurant_location(r), centroids), r] for r in restaurants]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    best_p = max(predictor, key = lambda p: p[1])
    return best_p[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            rate[name] = user_rating(user, name)
        else:
            rate[name] = predictor(r)
    return rate
    # END Question 9
...
def search(query, restaurants):
    ...
    result = []
    for r in restaurants:
        category = restaurant_categories(r)
