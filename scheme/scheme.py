...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        return SPECIAL_FORMS[first](rest, env, True)
    else:
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        ...
        if isinstance(operator, MacroProcedure):
            new_expr = operator.apply_macro(rest, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        f = formals
        v = vals
        while f != nil and v != nil:
            child.define(f.first, v.first)
            f = f.second
            v = v.second
        if f != nil or v != nil:
            raise SchemeError('unequivalent number of formal and val')
        return child
        # END PROBLEM 11
...
def do_define_form(expressions, env, tail = False):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        expr = Pair(target.second, expressions.second)
        func = do_lambda_form(expr, env, tail)
...
def do_if_form(expressions, env, tail = False):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.second.first, env, tail)
...
def do_and_form(expressions, env, tail = False):
    ...
    if expressions.second == nil:
        final = True
    first = scheme_eval(expressions.first, env, final and tail)
...
def do_let_form(expressions, env, tail = False):
    ...
    return eval_all(expressions.second, let_env, tail)
...
def do_define_macro(expressions, env, tail = False):
    ...
    check_form(first, 1)
    if not scheme_symbolp(first.first):
        raise SchemeError('Invaild Name: {0}'.format(first.first))
    formals = first.second
    values = expressions.second
    macro = MacroProcedure(formals, values, env)
    env.define(first.first, macro)
    return first.first
    # END Problem 21
